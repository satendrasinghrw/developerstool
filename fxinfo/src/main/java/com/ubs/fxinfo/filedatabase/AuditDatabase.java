...
public final class AuditDatabase {
	private final Logger logger = LoggerFactory.getLogger(AuditDatabase.class);
	...
	public List<AuditRecord> getAuditRecords() throws IOException {
		...
		final int noOfRecords = toBeReadLength%readLength == 0L ? 
				(int)(toBeReadLength/readLength) : 
	...
	public Map<String, AuditHistory> readAuditData() throws IOException {
		...
		final int readLength = AuditData.USER_DATA_LENGTH;
		final long toBeReadLength = dataFile.length();
		final int records = toBeReadLength%readLength == 0L ? 
				(int)(toBeReadLength/readLength) : 
				(int)(toBeReadLength/readLength) + 1;
